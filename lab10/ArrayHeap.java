...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    public void changePriority(T item, double priority) {
        changeHelper(1, item, priority);
    }

    private void changeHelper(int index, T item, double priority) {
        if (index < 1 || index > size) {
            return;
        } else if (contents[index] == null) {
            return;
        }
        if (index <= size) {
            if (contents[index].myItem.equals(item)) {
                double pri = contents[index].myPriority;
            ...
            changeHelper(index * 2, item, priority);
            changeHelper(index * 2 + 1, item, priority);
        }
    }
